...
@section('css')
    <style>
        ...
        @media only screen and (min-width: 1001px) div {
            display: block;
        }

        @media (min-width: 1024px) .lg\  :lex-row {
            flex-direction: row;
        }

        @med i (min-width: 768px) .md\:gap-0 {
            gap: 0;
        }
        ...
        @med i only screen ad (min-width: 1001px) div {
            display: block;
        }

        .shadow-sm,
        .shadow-xl {
            b x-shadow: var(--tw-ring-offset-shadow,
        ...
        .shadow-sm {
            ...
            - tw-shadow-colo red: 0 1px 2px 0 var(--t -shadow-color);
        }
        ...
        #seconds,
        .total {
            background: linear-gradient(to right, rgb(249, 45, 45), rgb(245, 121, 30));
                ...
                <div>
                        ...
                        <div class="py-4">
                            ...
                            @if ($data->status_pembayaran == 'Belum Lunas')
                                @if (Str::upper($data->metode_pembayaran) == 'QRIS' ||
                                    ...
                                    <div class="col-lg-7 d-print-none">
                                        <a href="{{ $data->no_pembayaran }}" download="">
                                            <img src="{{ $data->no_pembayaran }}" alt="qrcode" class="img-fluid"
                                ...
                                @elseif(Str::upper($data->metode_pembayaran) == 'LA')
                                    <a class="btn btn-primary btn-sm" href="{{ $data->no_pembayaran }}">KLIK DISINI BAYAR
                                        VIA LINKAJA</a>
                    ...
                    @elseif(Str::upper($data->metode_pembayaran) == 'OVOADMIN' ||
                        ...
                        <kbd class="bg-dark" data-toggle="tooltip" data-placement="bottom" id="paycode"
                            title="Click to copy" style="margin-left: 50px;">{{ $data->no_pembayaran }}
                        </kbd>
                        </h6>
                        <br>
                    </div>
                @else
                    <kbd class="bg-dark" data-toggle="tooltip" data-placement="bottom" id="paycode" title="Click to copy"
                        style="margin-left: 50px;">{{ $data->no_pembayaran }}
                    </kbd>
                    </h6>
                    <br>
                </div>
                ...
                @if ($data->status_pembayaran == 'Belum Lunas')
                    @if (Str::upper($data->metode_pembayaran) == 'QRIS' ||
                        ...
                        <div class="mb-4 flex justify-center">
                            <button type="button"
                                class="inline-flex items-center rounded-full border border-transparent px-6 py-3 text-sm font-semibold leading-4 shadow-sm focus:outline-none"
                    ...
                    @elseif(Str::upper($data->metode_pembayaran) == 'OVOADMIN' ||
                        ...
                        <div class="mb-4 flex justify-center">
                            <button type="button"
                                class="inline-flex items-center rounded-full border border-transparent px-6 py-3 text-sm font-semibold leading-4 shadow-sm focus:outline-none"
                                title="Click to copy" id="paycode" onclick="copyToClipboard()">Click To Copy
                            </button>
                        </div>
                    @else
                        <div class="mb-4 flex justify-center">
        ...
        <div class="min-w-0 flex-1 xl:flex">
            <div class="lg:min-w-0 lg:flex-1">
                <div class="h-full py-6">
                    <div
                        class="flex flex-col lg:flex-row md:flex-row gap-3 lg:gap-0 md:gap-0 justify-between rounded-2xl p-6 shadow-sm css-scrciq">
                        <div>
                    ...
                    <div class="mt-4 overflow-hidden rounded-2xl shadow-sm css-scrciq">
                        ...
                        <div class="flex items-center gap-4 px-4 py-5 sm:px-6">
                            <img alt="{{ $data->nama }}"
                                srcset="{{ $data->thumbnail }} 1x, {{ $data->thumbnail }} 2x"
                                src="{{ $data->thumbnail }}" width="500" height="500" decoding="async"
                                data-nimg="1" class="h-10 w-10 rounded-lg md:h-16 md:w-16 object-cover" loading="lazy"
                            ...
                            <dl class="border-b border-[#626274] p-4 pb-8">
                                ...
                                @elseif($data->tipe_transaksi == 'dm_vilog')
                                    ...
                                    <div class="py-2 sm:grid sm:grid-cols-3 sm:gap-3 sm:py-2">
                                        ...
                                        <dd class="mt-1 text-sm font-bold sm:col-span-2 sm:mt-">
                                            {{ $data->password_vilog }}
                                        </dd>
                                    </div>
                                    <div class="py-2 sm:grid sm:grid-cols-3 sm:gap-3 sm:py-2">
                                ...
                                @else
                                    <div class="py-2 sm:grid sm:grid-cols-3 sm:gap-3 sm:py-2">
                                        ...
                                        <dd class="mt-1 text-sm font-bold sm:col-span-2 sm:mt-">{{ $data->user_id }}
                                            {{ $data->zone != null ? '(' . $data->zone . ')' : '' }}
                                        </dd>
                                    </div>
                                    <div class="py-2 sm:grid sm:grid-cols-3 sm:gap-3 sm:py-2">
                                        <dt class="text-sm font-medium">Nickname
                                    ...
                                    <h5 class="font-extrabold ">
                                        <td class="right text-right">
                                            <strong class="total">
                                                ...
                                                <i class="fas fa-copy" type="button" title="Click to copy"
                                                    id="paycode" onclick="copyToClipboard()">
                                                </i>
                                            </strong>
                                        </td>
                                    </h5>
                    ...
                    <div class="mt-8 text-center">
                        <p class="mx-auto max-w-xl text-sm">
                            Jika Kamu Mengalami Kendala Pembayaran Atau Adanya Keluhan Terkait Pembayaran Anda Dapat
                            Menghubungi Kami Melalui&nbsp;
                            <a href="{{ !$config ? '' : $config->url_wa }}" class="capitalize text-success"
                                target="_blank" rel="noreferrer">WhatsApp
                            </a>
                        </p>
                    </div>
                </div>
